...
| RC credentials absent from process arguments | Locally verified; native CI pending | Mount commands use an owner-private BCrypt htpasswd file instead of `--rc-user`/`--rc-pass`. Runtime tests verify the raw random credential is absent from argv, the hash authenticates it, Unix mode is `0600`, and lifecycle cleanup preserves the file only with deliberately retained unverifiable state. |
| Native SFTP host-key validation fails closed | Locally verified; native CI pending | Keyscan must exit successfully and return normalized keys; remote construction rejects a missing, unreadable, marker-only, or unrelated host-key file unless an entry binds the resolved host and port with a supported key type and key data. Focused and complete core tests cover empty/nonzero scans, established pins, malformed/mismatched paths, and unaffected OpenSSH/Interactive transports. |
| App-command slow-client resistance | Locally verified; native CI pending | Four fixed pre-authentication workers, a bounded queue, an accept-time 500 ms total request deadline covering read and JSON parsing, the existing 64 KiB limit, and serialized authenticated callbacks prevent one partial request from occupying the accept loop. Tests cover slow and drip-fed input, accept-time and parse-time expiry, oversized and exact-limit input, invalid tokens, valid delivery while a slow client is active, callback shutdown ordering, startup cleanup, and bounded drop. |
