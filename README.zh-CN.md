...
如果 `ssh-keyscan` 失败（包括部分 Windows OpenSSH 版本的密钥交换算法兼容问题），程序会尝试普通 SSH 握手：禁用认证，只使用隔离的临时信任文件，不使用你的密码、私钥、SSH 代理或 SSH 配置。探测结束后清理临时文件。两种方式都无法获取公钥时，弹窗显示具体原因，并提供「重试」和「取消」。
