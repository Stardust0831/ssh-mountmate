...
Release 工作流会下载目标平台和架构的固定版本官方 rclone，校验 SHA-256 后把它放在 Rust 程序旁边。运行时 SSH MountMate 会再次校验内置摘要，并在应用数据目录生成按内容摘要命名的托管副本。为迁移兼容，显式配置和已有旧版托管副本仍然可用；源码构建最后还可以使用系统 `PATH` 中兼容的 rclone。
...
这些发布包由六个原生 GitHub Actions runner 从 Rust 工作区构建。Windows 和 Linux ZIP 内是一个嵌入并校验官方 rclone 的可执行文件；macOS ZIP 内是原生 `SSH MountMate.app`，rclone 和许可证声明位于应用包内。
...
`AMD64` / `x86_64` 选择 `x64` 包，`ARM64` / `arm64` / `aarch64` 选择 `arm64` 包。Windows 和 Linux 每个架构只提供一个规范 onefile 包，可执行文件首次使用时会把内嵌 rclone 物化为按内容摘要命名的受管副本。macOS 每个架构只提供一个原生 `.app` 包。发行矩阵固定为六个 ZIP，不再分别提供 onefile 和 onedir 变体。
