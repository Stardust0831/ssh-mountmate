...
这些发布包由六个原生 GitHub Actions runner 从 Rust 工作区构建。请完整解压，并保留相邻的 `bin` 和许可证文件。macOS 包内是 `SSH MountMate.app`。
...
应用内更新器会下载匹配当前平台的原生 GitHub Release 附件，校验大小和 SHA-256，拒绝不安全的 ZIP 路径，把新的目录包或 macOS 应用暂存在当前安装目录旁，并在用户确认后重启 SSH MountMate。新版本通过启动健康握手后才提交更新；超时或失败会恢复并重新启动旧版本。GUI 重启期间，已有 rclone 挂载和上传会继续运行。
